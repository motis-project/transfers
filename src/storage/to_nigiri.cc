...
hash_map<location_key_t, n::location_idx_t> to_location_key_to_idx_(
    n::vector_map<n::location_idx_t, geo::latlng> coords) {
  auto res = hash_map<location_key_t, n::location_idx_t>{};

  for (auto const [i, coord] : utl::enumerate(coords)) {
    res.emplace(location(coord).key(), n::location_idx_t{i});
  }
...
nigiri_transfers build_nigiri_transfers(to_nigiri_data const& data) {
  ...
  auto const& single_update = [&](transfer_result const& tres) {
    ...
    for (auto [to_nloc, info] : utl::zip(tres.to_locs_, tres.infos_)) {
      if (data.profile_name_to_tt_idx_.count(
              data.profile_key_to_profile_name_.at(tres.profile_)) == 0 ||
          location_key_to_idx.count(tres.from_loc_) == 0 ||
      ...
      auto const from_idx = location_key_to_idx.at(tres.from_loc_);
